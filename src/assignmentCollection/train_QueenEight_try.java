...
class Stack5 {
	...
	private List<Point6> data; // 스택용 배열
	// private List<T> data;
	...
	public Stack5(int capacity) {
		...
		top = 0;
		data = new ArrayList<Point6>();
	}

	// --- 스택에 x를 푸시 ---//
	public boolean push(Point6 x) throws OverflowGenericStackException {
		if (isFull()) {
			throw new OverflowGenericStackException("StackIsFull");
		}
		top++;
		data.add(x);
	...
	public Point6 pop() throws EmptyGenericStackException {
		if (isEmpty()) {
			throw new EmptyGenericStackException("StackIsEmpty");
		}
		top--;
	...
	public Point6 peek() throws OverflowGenericStackException {
		if (isEmpty()) {
			throw new OverflowGenericStackException("StackisEmpty");
		}
...
public class train_QueenEight_try {

	public void EightQueen(int[][] d) {
		int ix = 0;
		int iy = 0;
		...
		d[ix][iy] = 1;
		int count = 1;
		Stack5 st = new Stack5(100);
		Point6 p = new Point6(ix, iy);
		int numOfSolution = 0;
		...
		try {
			while (true) {
				// ix와 iy의 의미...?
				// ix와 iy는 현재 퀸의 위치의 다음열?

				if (st.isEmpty() && ix == 8) { // ix가 8이면 8개 배치 완료, stack이 empty가 아니면 다른 해를 구한다
					break;
				}
				if ((iy = nextMove(d, ix, iy)) == -1) {// 다음 이동할 열을 iy로 주는데 -1이면 더이상 이동할 열이 없음을 나타냄 백트래킹
					p = st.pop();
					ix = p.getIx();
					iy = p.getIy();
					d[ix][iy] = 0;
				...
				{
					numOfSolution++;
					System.out.println(numOfSolution);
					showQueens(d);
				}
			}
...
//배열 d에서 현재 위치(row,col)에 대하여 다음에 이동할 위치 nextCol을 반환, 이동이 가능하지 않으면 -1를 리턴
	...
	public static void main(String[] args) {
		train_QueenEight_try tq = new train_QueenEight_try();
