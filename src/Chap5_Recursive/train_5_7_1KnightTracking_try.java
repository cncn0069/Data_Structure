...
public class train_5_7_1KnightTracking_try {
    ...
    private static boolean solveKnightTracking(int startX, int startY) {
        ...
        while (!stack.isEmpty()) {
            ...
            // 8가지 방향으로 나이트 이동 시도
           if(stack.peek().moveToward <= 7)
           {
        	   nextX = curX + moves[stack.peek().moveToward].a;
        	   nextY = curY + moves[stack.peek().moveToward].b;
        	   //이동할 수 있는지 
        	   if(isSafe(nextX, nextY))
        	   {
        		   
        		   board[nextX][nextY] = visitPoint++;
