...
import assignment.chap10.SimpleObject5;
...
class LinkedList2 {
	...
	// 전달된 element를 찾을 때 comparator 객체를 사용한다
	{
		...
		while (current != null) {
			if (cc.compare(current.data, element) == 0) {
				q.link = current.link;
				return Integer.parseInt(current.data.getNo());
			}
			q = current;
	...
	// 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		// 무엇을 기준으로 넣을 것이냐.. 당연히 Comparator를 기준으로지?!
		// cc를 기준으로 정렬하는 법...
		// a가 크면 1 작으면 -1 같으면 0
		...
		while (curNode != null) {
			// newNode가 더 작으면
			// 같은 경우도?
			if (cc.compare(newNode.data, curNode.data) <= 0) {
...
public class train_try8_2ObjectLinkedList {
