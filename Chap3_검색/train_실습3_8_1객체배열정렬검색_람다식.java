...
public class train_실습3_8_1객체배열정렬검색_람다식 {
	...
	private static void sortData(Fruit4[] arr, Comparator<Fruit4> cc_price) {
		...
		for(int i = 0; i < arr.length ; i++)
		{
			...
			{
				if(cc_price.compare(arr[min], arr[j]) > 0) {
					min = j;
				}
			}
			swap(arr,i,min);
	...
	private static void swap(Fruit4[] data,int i,int j) {
		Fruit4 temp = data[i];
		data[i] = data[j];
		data[j] = temp;
	}
	...
	private static void showData(String msg,Fruit4[] data) {
	...
	public static int binarySearch(Fruit4[] arr, Fruit4 newFruit4,Comparator<Fruit4> cc) {
		int pl = 0;
		int pr = arr.length-1;
		int pc = 0;
		...
		do {
			...
			if(cc.compare(arr[pc], newFruit4) == 0) {
				return pc;
			}else if(cc.compare(arr[pc], newFruit4) < 0){
				pl = pc + 1;
			}else {
				pr = pc + 1;
			}
